      ...
      <div className="w-full max-w-md rounded-3xl bg-card p-8 shadow-elegant">
        <div className="mb-6 flex flex-col items-center text-center">
          ...
          <p className="text-xs uppercase tracking-widest text-muted-foreground">
            {mode === "forgot" ? "Reset your password" : "Administrator Access"}
          </p>
        </div>
        <form onSubmit={submit} className="space-y-3">
          ...
          {info && <div className="rounded-lg bg-primary/10 px-3 py-2 text-xs text-primary">{info}</div>}
        ...
        <div className="mt-4 text-center text-xs text-muted-foreground">
          {mode === "signin" && (
            <>New here? <button onClick={() => switchMode("signup")} className="text-primary hover:underline">Create an account</button></>
          )}
          {mode === "signup" && (
            <>Already registered? <button onClick={() => switchMode("signin")} className="text-primary hover:underline">Sign in</button></>
          )}
          {mode === "forgot" && (
            <>Remembered it? <button onClick={() => switchMode("signin")} className="text-primary hover:underline">Back to sign in</button></>
          )}
